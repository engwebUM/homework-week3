...
RSpec.describe Bowling do
	...
	context '#roll' do
		it 'invalid number of pins' do
			expect(game.run(11)).to match /Error1/	
		end
		...
		it 'The game is finished. No more launches. (Case 1)' do
			...
			game.run(1)
			game.run(1)
			expect(game.run(1)).to match /Error2_1/
		end
		...
		it 'The number of pins dropped in the frame exceeded the limit of tens. (Case 2)' do
			...
			game.run(0)
			game.run(4)
			expect(game.run(9)).to match /Error4_2/
		end
	end
